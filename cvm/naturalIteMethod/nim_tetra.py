...
class process(object):
    ...
    def __init__(self, data):
        ...
        self.e_ijkl[1, 0, 0, 0] = \
            -(6 * data.int_pair + 8 * data.int_trip + 2 * data.int_tetra)
        self.e_ijkl[1, 1, 0, 0] = \
            -(12 * data.int_pair + 24 * data.int_trip + 6 * data.int_tetra)
        self.e_ijkl[1, 1, 1, 0] = \
            -(6 * data.int_pair + 16 * data.int_trip + 6 * data.int_tetra)
    ...
    def __eta_ijkl(self, i, j, k, l):
        """
        η_ijkl = exp[-β*e_ijkl + (β/8)(mu + mu_j + mu_k + mu_l)]
                    * X^(-5/8)
                    * Y^(1/2)
        X = x_i * x_j * x_k * x_l
        Y = y_ij * y_ik * y_il * y_jk * y_jl * y_kl
        """
        eta_ijkl = np.exp(-self.beta * self.e_ijkl[i, j, k, l] +
                          (self.beta / 8) *
                          (self.mu[i] + self.mu[j] +
                           self.mu[k] + self.mu[l]))
        X = np.power(
            (self.x_i[i] * self.x_i[j] * self.x_i[k] * self.x_i[l]), -5 / 8)
        Y = np.power((self.y_ij[i, j] * self.y_ij[i, k] * self.y_ij[i, l]
                      * self.y_ij[j, k] * self.y_ij[j, l] * self.y_ij[k, l]),
                     1 / 2)
        return eta_ijkl * X * Y
    ...
    def __z_ijkl(self):
        ...
        eta_sum = eta_2222 + eta_2221 + eta_2211 + eta_2111 + eta_1111
        self.lam = -np.log(eta_sum) * 2 / self.beta
    ...
    def __run(self):
        ...
        self.x_i[0] = 1 * self.z_ijkl[0, 0, 0, 0] + \
            3 / 4 * self.z_ijkl[1, 0, 0, 0] + \
            3 / 6 * self.z_ijkl[1, 1, 0, 0] + \
            1 / 4 * self.z_ijkl[1, 1, 1, 0]
        self.x_i[1] = 1 - self.x_i[0]

        # y_ij
        self.y_ij[0, 0] = 1 * self.z_ijkl[0, 0, 0, 0] + \
            2 / 4 * self.z_ijkl[1, 0, 0, 0] + \
            1 / 6 * self.z_ijkl[1, 1, 0, 0]
        self.y_ij[1, 0] = self.y_ij[0, 1] = 1 * self.z_ijkl[1, 0, 0, 0] + \
            2 / 6 * self.z_ijkl[1, 1, 0, 0] + \
            1 / 4 * self.z_ijkl[1, 1, 1, 0]
        self.y_ij[1, 1] = 1 * self.z_ijkl[1, 1, 0, 0] + \
            2 / 4 * self.z_ijkl[1, 1, 1, 0] + \
