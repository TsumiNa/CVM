...
def __eta_fp(self, i, j, k, l, m, n):
    """
    η_ijklmn = exp[-β*e_ijklmn +
                    (β/14)(mu_i + mu_j + mu_k + mu_l + mu_m + mu_n)]
                * M61^(5/6)
                * M51^(1/2)
                * M21^(-4/11)
                * M22^(-1/4)
    ...
    # M61
    M61 = self.m41_[i, j, k, l] * self.m41_[m, n, k, l]

    # M51
    M51 = self.m31_[i, m, k] * self.m31_[i, m, l] * \
...
def process(self):
    ...
    while not it.finished:
        ...
        dt_[i, j, k, l, m, n] = __eta_fp(self, i, j, k, l, m, n)
